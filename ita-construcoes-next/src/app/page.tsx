          ...
          {latest.map(p => (
            <article key={p.slug} className="border border-gray-100 rounded-2xl overflow-hidden bg-white shadow-sm">
              ...
              <div className="p-5">
                ...
                <Link href={`/blog/${p.slug}`} className="text-brand text-sm font-medium inline-block mt-3">Ler artigo →</Link>
              </div>
            </article>
          ))}
        </div>
        <div className="mt-6">
          <Link href="/blog" className="text-brand font-medium">Ver todos os artigos →</Link>
        </div>
      </Section>
